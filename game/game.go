...
import (
	...
	"github.com/invokerw/demo/game/internal/config"
...
func main() {
	...
	defer agent.Close()
	err = agent.Subscribe(g.NATS_SERVER_SUBJECT, func(msg *nats.Msg) {
		logger.Info(g.NATS_SERVER_SUBJECT, "Subscribe receive:", string(msg.Data))
	})
	if err != nil {
		logger.Error(err)
		return
